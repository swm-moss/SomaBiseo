...
프로젝트 대화에서 확정된 최신 맥락은 [Conversation Notes](docs/conversation.md)를 우선 확인합니다.

## 절대 하지 않을 것

1. 공식 소마 앱처럼 보이는 문구를 쓰지 않습니다.
2. SOMA 포털 비밀번호를 저장하지 않습니다.
3. 사용자 명시 요청 없는 신청, 취소 자동 반복 실행이나 매크로성 동작을 만들지 않습니다.
4. 무단 대량 크롤링을 만들지 않습니다.
5. 공식 로고를 무단으로 쓰지 않습니다.
6. 랭킹, 추천 알고리즘을 MVP 1차에 넣지 않습니다. 후기는 별점/수정/삭제 없이 본문만 받는 1차 범위로 포함합니다.
...
- 모바일 퍼스트로 만듭니다.
- 로딩, 에러, 빈 상태를 구현합니다.
- 서버 상태와 API 캐싱은 TanStack Query를 사용하고, 기본 QueryClient 정책은 `frontend/src/shared/api/query-client.ts`를 따릅니다.
- Zustand는 임시 세션, 관심 저장, 읽음 같은 브라우저 로컬 상태에만 사용합니다.
