...
const features = [
  ...
  },
  {
    ...
    title: "관심사 기반 추천",
    description:
      "AI, 프론트, 백엔드처럼 설정한 관심사에 맞는 일정을 먼저 보여줍니다.",
  },
] as const;
...
export function LandingPage() {
  return (
    <main className="min-h-screen bg-[#f3f4f8] text-foreground">
      <section>
        <div className="sb-landing-stage relative isolate min-h-svh w-full overflow-hidden">
          ...
          <div
            aria-hidden="true"
            className="sb-landing-machine absolute left-1/2 top-[50%] z-10 w-[300px] sm:top-[52%] sm:w-[440px] lg:top-[53%] lg:w-[560px]"
          >
          ...
          <div className="mt-12 divide-y divide-border/80 border-y border-border/80">
            {features.map((feature) => {
              ...
              return (
                <div
                  key={feature.title}
                  className="grid gap-5 py-7 sm:grid-cols-[220px_1fr] sm:items-center"
                >
      ...
      <section className="bg-[#101828] px-5 py-14 text-white sm:px-6 lg:px-8">
        <div className="mx-auto flex max-w-[1120px] flex-col gap-8 lg:flex-row lg:items-center lg:justify-between">
          <div>
            <p className="text-[15px] font-bold leading-[22px] text-[#8ec5ff]">
              MVP 기준
            </p>
