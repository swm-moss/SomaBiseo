...
| `GOOGLE_CALENDAR_CLIENT_ID` | `********.apps.googleusercontent.com` | Google OAuth client ID. 로그인과 Calendar 연결에 함께 사용 |
| `GOOGLE_CALENDAR_CLIENT_SECRET` | `********` | Google OAuth client secret |
| `GOOGLE_CALENDAR_REDIRECT_URI` | `https://backend-production-76bf.up.railway.app/api/calendar/google/callback` | Google OAuth Authorized redirect URI |
| `GOOGLE_CALENDAR_FRONTEND_REDIRECT_URI` | `https://somabiseo.vercel.app/settings` | Calendar 연결 후 돌아갈 기본 프론트 URL |
...
| `SOMABISEO_INVITE_CODE` | `******` | 첫 로그인 구성원 인증에 사용하는 6자리 초대 코드 |
| `SOMABISEO_INVITE_MAX_FAILED_ATTEMPTS` | `5` | 초대 코드 연속 실패 허용 횟수 |
| `SOMABISEO_INVITE_LOCK_MINUTES` | `10` | 초대 코드 실패 제한 지속 시간 |
...
Google OAuth client secret도 운영 환경변수에만 등록하고 코드, 문서, PR 본문에 원문을 남기지 않는다.
Google OAuth Authorized redirect URI는 백엔드 callback 하나만 등록한다. 로그인은 `openid email profile`, Calendar 연결은 Calendar 권한을 추가 요청하고 둘 다 같은 callback에서 앱 세션으로 정리한다.
초대 코드는 운영 환경변수에만 등록하고 코드, 문서, PR 본문에 원문을 남기지 않는다.
...
5. Google Cloud Console OAuth client의 Authorized redirect URI에 Railway callback URL을 등록한다.
6. Vercel 환경변수 `NEXT_PUBLIC_API_BASE_URL`에 Railway 백엔드 도메인 + `/api`를 등록한다.
7. GitHub secrets와 variables를 등록한다.
8. `main`에 병합한 뒤 `CI`, `Deploy Production` 워크플로우가 차례로 성공하는지 확인한다.
